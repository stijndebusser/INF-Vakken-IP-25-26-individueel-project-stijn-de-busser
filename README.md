# Individueel Project: AR Feedforward Visualisatie voor Robotarmen

**Academiejaar:** 2025-2026  
**Student:** Stijn De Busser  
**Promotors:** Prof. Dr. Kris Luyten, Prof. Dr. Raf Ramakers, Dries Cardinaels

---

## 📋 Overzicht

Dit project richt zich op het ontwikkelen van **feedforward-visualisaties** voor robotarmen in een manufacturing-omgeving met behulp van Augmented Reality (AR). In plaats van feedback achteraf te geven, visualiseert feedforward proactief toekomstige bewegingen, trajecten en potentiële collisies voordat deze plaatsvinden.

### Kerncomponenten

Het onderzoeksproject addresseert drie belangrijke uitdagingen:

1. **Traject-voorspelling**: Visualisatie van het volledige bewegingspad van de robotarm
2. **Collision awareness**: Proactieve detectie en visualisatie van potentiële collisies
3. **Spatial awareness**: Verbeterde 3D-interpretatie via AR passthrough-laag

---

## 📚 Documentatie

- [**Probleembeschrijving**](docs/probleembeschrijving.md) - Uitgebreide beschrijving van het onderzoeksprobleem en context
- [**Literatuurstudie**](docs/literatuurstudie.md) - Overzicht van relevante literatuur en onderzoek
- [**Verslagen**](docs/verslagen.md) - Vergaderverslagen en opvolgingsmomenten
- [**Timesheet**](docs/timesheet.md) - Tijdsregistratie van activiteiten
- [**Onderzoeksvragen**](docs/onderzoeksvragen.md) - Onderzoeksvragen en doelen

---

## 🎯 Doel

Het doel is om operators te ondersteunen bij het instellen, programmeren en monitoren van robotarmen door visuele feedforward te bieden. Dit leidt tot:

- Kortere programmeertijden
- Verhoogde veiligheid
- Verminderd risico op schade
- Verbeterde communicatie tussen planning en operator

---

## 🔧 Status

Project in onderzoeksfase - momenteel bezig met probleemdefiniëring en literatuurstudie.